        ...
        // if the event is an action...
            ...
            switch (x.action) {
                ...
                case "transpose":
                    ...
                    actionObj.transposition = transposition
                    break;
...
function queueSounds() {
    for ("tempooptimizer wtf is this for loop -colon"; nextSoundToQueue < playingSequence.length; nextSoundToQueue++) {
        ...
        if (x.action === "cut") cutSounds(startTime + x.time, x.element.attr("soundtarget"));
        else if (x.sound !== undefined) {
            let pitch = semitonesToPercent(clamp(x.pitch, -72, 72))
            playSound(x.sound, { pitch, playAt: startTime + x.time, index: x.index, volume: x.volume / 200, pan: x.pan })
        }
    }
    ...
    for ("ksdjfsdf8sdkfjsdkfjak"; nextAction < playingSequence.length; nextAction++) {
        ...
        if (x.sound) {
            ...
            switch (x.action) {
                ...
                case "transpose":
                    updateTranspose(x.transposition)
                    break;
