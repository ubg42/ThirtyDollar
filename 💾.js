...
function generateSequenceFile() {
    ...
    console.info(`Exporting sequence... (${$('#sequence div').length} icons)`)
    let sequenceList = []
    let toAdd = ""
    $('#sequence div').each(function() {
        ...
        if (x.attr('sound')) {
            ...
            let soundID = foundSnd ? (foundSnd.useID ? foundSnd.id : (foundSnd.emoji || foundSnd.id)) : (snd || "_pause")
            let hasPitch = (pitch && pitch != 0)
            toAdd = (hasPitch ? `${soundID}@${pitch}` : soundID)
            if (vol && vol != 100) toAdd += `%${vol}`
            if (pan) toAdd += `^${pan}`
        }
            ...
            else if (x.attr('soundtarget')) {
                actionStr += `@${x.attr('soundtarget')}`
            }
    ...
    console.info(`Exporting took ${+((Date.now() - startingTime) / 1000).toFixed(2)}s`)
    return finalString
}
...
function loadSequence(data) {
    let startingTime = Date.now()
    console.info(`Loading sequence... (size: ${+(data.length / 1000).toFixed(2)} KB)`)
    try { 
        let sequenceData = "";
        (data || "").replace(/\s/g, "").split("|").forEach(x => {
            let [data, count] = x.split("=")
            let [main, param, num] = data.split("@")
            if (!main || !data) return
            let isAction = main.startsWith("!")
            let element = null
            if (isAction) {
                main = main.slice(1) // remove !
                element = $(`#actions .action[action=${main}]`).first().clone()
                if (!element.length) return
                let actionData = actions.find(x => x.name == main)
                if (actionData.twoValues) {
                    if (!element.find("p").length) element.append("<p></p>")
                    for (let i=0; i<(count || 1); i++) sequenceData += addAdvancedAction(actionData.name, param.split(","), element, true).prop('outerHTML')
                    return    
                }
                else if (actionData.amount || actionData.isTarget) {
                    if (actionData.isTarget && !param) param = 1 
                    if (!element.find("p").length) element.append("<p></p>")
                    if (num == "x") num = "multiply"
                    else if (num == "/") num = "divide"
                    else if (num == "+") num = "add"
                    for (let i=0; i<(count || 1); i++) sequenceData += addAction(main, +param, num, element, true).prop('outerHTML');
                    return
                }
                else if (actionData.soundTarget && param) {
                    let targetSound = getSound(param)
                    if (targetSound) addSoundTarget(element, targetSound)
                }
            }
            else {
                let splitter = data.search(/[@%^]/)
                let id = splitter == -1 ? data : data.slice(0, splitter)
                let pitchVal; let volVal; let panVal;

                let params = data.slice(splitter).split(/(?=[@%^])/) // split without removing from string
                params.forEach(x => {
                    if (x.charAt(0) == "@") pitchVal = x.slice(1)
                    else if (x.charAt(0) == "%") volVal = x.slice(1)
                    else if (x.charAt(0) == "^") panVal = x.slice(1)
                })
                
                let foundSound = getSound(id)

                element = $(`.sound[str=${foundSound ? foundSound.id : "_pause"}]`).first().clone()
                if (foundSound) {
                    if (pitchVal && pitchVal != 0) {
                        pitchVal = clampPitch(pitchVal)
                        element.attr('pitch', pitchVal)
                        element.append(`<p>${pitchVal > 0 ? "+" : ""}${pitchVal}</p>`)
                    }
                    if (volVal && volVal != 100) {
                        volVal = clampVolume(volVal)
                        element.attr('vol', volVal)
                        element.append(`<vol>${volVal}%</vol>`)
                    }
                    if (panVal && panVal != 0) {
                        panVal = clampPan(panVal)
                        element.attr('pan', panVal)
                        element.append(`<pan>${panVal / 10}</pan>`)
                    }
                }
        ...
        setUnsavedChanges(false)
        cancel()
        console.info(`Loading took ${+((Date.now() - startingTime) / 1000).toFixed(2)}s`)
    }
